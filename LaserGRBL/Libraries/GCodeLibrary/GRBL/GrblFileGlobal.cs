...
namespace GRBLLibrary
{
	...
	public class GrblFileGlobal
	{
		...
		public void ReScale(ProgramRange layerRange)
		{
            ...
            // Spindle range
            globalRange.SpindleRange.S.Min = Math.Min(globalRange.SpindleRange.S.Min, layerRange.SpindleRange.S.Min);
            globalRange.SpindleRange.S.Max = Math.Max(globalRange.SpindleRange.S.Max, layerRange.SpindleRange.S.Max);
		...
		public void DrawJobPreview(GrblCommand.StatePositionBuilder spb, Color startColor, Color defaultColor, List<GrblCommand> grblCommands)
		{
			...
			foreach (GrblCommand cmd in grblCommands)
			{
				...
				{
					cmd.BuildHelper();
					spb.AnalyzeCommand(cmd);


					if (spb.TrueMovement())
					{

						//Color not right here
						...
						int d = spb.GetCurrentAlpha(globalRange.SpindleRange);
                        //(int)spb.S.Number

                        // globalRange.SpindleRange has not been set?


                        Color linecolor = Color.FromArgb(
							//(int)spb.S.Number,							// Alpha, must be between 0 and 255, Spindle speed could be anything so we need to scale the range
							d,
							firstline ? startColor : spb.LaserBurning ? defaultColor : ColorScheme.PreviewOtherMovement
						);
		...
		{
			using (Pen pen = GetPen(Color.FromArgb(255, green: 0, 0)))
			{
