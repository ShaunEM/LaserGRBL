...
namespace LaserGRBLPlus.SvgConverter
{
    public class GCodeFromSVG
	{
        ...
        public Firmware firmwareType = Setting.App.FirmwareType;
        public float gcodeSpindleSpeed = 999; // Spindle speed to apply





        //public float gcodeXYFeed = 1999;        // XY feed to apply for G1
        public bool SupportPWM = Setting.App.SupportHardwarePWM;
		
		
        public string gcodeSpindleCmdOn = Setting.App.GrayScaleConversionGcodeLaserOptionsLaserOn; //", "M3"); // Spindle Command M3 / M4

        private string formatCode = "0";
		private string formatNumber = "0.###";
		private bool gcodeDragCompensation = false;
		private float gcodeDragRadius = 5;
		private float gcodeDragAngle = 30;
		//private static bool gcodeSpindleToggle = true; // Switch on/off spindle for Pen down/up (M3/M5)
		
		private string gcodeSpindleCmdOff = Setting.App.GrayScaleConversionGcodeLaserOptionsLaserOff; //", "M5"); // Spindle Command M3 / M4
		private bool gcodeCompress = true;      // reduce code by avoiding sending again same G-Nr and unchanged coordinates
		private bool gcodeNoArcs = false;        // replace arcs by line segments
		private float gcodeAngleStep = 1.0f;
		private int mDecimalPlaces = 3;
		private int rapidnum = 0;


		//public static void setup(GrblCore core, int layerIndex)
		public void Reset()
		{
			// If Support PWM use S command instead of M3-M4 / M5
			//SupportPWM = GlobalSettings.GetObject("Support Hardware PWM", true); 
			...
			//if (SupportPWM)
			//         {
			//	gcodeSpindleSpeed = core.ProjectCore.layers[layerIndex].LayerSettings.GetObject("GrayScaleConversion.Gcode.LaserOptions.PowerMax", 255);
			//         }
			//else
			//         {
		...
		public void setRapidNum(int num)
		{ rapidnum = num; }
		...
		public int getIntGCode(char code, string tmp)
		{
		...
		public string getStrGCode(char code, string tmp)
		{
			int cmt = tmp.IndexOf("(");
			if (cmt == 0)
		...
		public int getIntValue(char code, string tmp)
		{
		...
		public string getStringValue(char code, string tmp)
		{
		...
		public string frmtCode(int number)      // convert int to string using format pattern
		{ return number.ToString(formatCode); }
		public string frmtNum(float number)     // convert float to string using format pattern
		{ return number.ToString(formatNumber); }
		public string frmtNum(double number)     // convert double to string using format pattern
		{ return number.ToString(formatNumber); }

		private StringBuilder secondMove = new StringBuilder();
		private bool applyXYFeedRate = true; // apply XY feed after each Pen-move

		public void SpindleOn(StringBuilder gcodeString, string cmt = "")
		{
		...
		public void SpindleOff(StringBuilder gcodeString, string cmt = "")
		{
			if (cmt.Length > 0) cmt = string.Format(" ({0})", cmt);
		...
		internal void PutInitialCommand(StringBuilder gcodeString)
		{
			if (SupportPWM)
			{
		...
		internal void PutFinalCommand(StringBuilder gcodeString)
		{
			if (SupportPWM)
		...
		public void PenDown(StringBuilder gcodeString, string cmto = "")
		{
			string cmt = cmto;
			drag1stMove = true;
			origFinalX = lastx;
		...
		public void PenUp(StringBuilder gcodeString, string cmto = "")
		{
			string cmt = cmto;
			drag1stMove = true;
			origFinalX = lastx;
		...
		public float lastx, lasty, lastz, lastg = -1, lastf, lasts;
		public bool lastMovewasG0 = true;
		public void MoveTo(StringBuilder gcodeString, Point coord, string cmt = "")
		{
			MoveSplit(gcodeString, 1, (float)coord.X, (float)coord.Y, applyXYFeedRate, cmt);
		}
		public void MoveTo(StringBuilder gcodeString, float x, float y, string cmt = "")
		{ MoveSplit(gcodeString, 1, x, y, applyXYFeedRate, cmt); }
		public void MoveTo(StringBuilder gcodeString, float x, float y, float z, string cmt = "")
		{ MoveSplit(gcodeString, 1, x, y, z, applyXYFeedRate, cmt); }
		public void MoveToRapid(StringBuilder gcodeString, Point coord, string cmt = "")
		{
			Move(gcodeString, rapidnum, (float)coord.X, (float)coord.Y, false, cmt); lastMovewasG0 = true;
		}
		public void MoveToRapid(StringBuilder gcodeString, float x, float y, string cmt = "")
		{
		...
		private void MoveSplit(StringBuilder gcodeString, int gnr, float x, float y, bool applyFeed, string cmt)
		{
		...
		private void MoveSplit(StringBuilder gcodeString, int gnr, float finalx, float finaly, float? z, bool applyFeed, string cmt)
		{
		...
		private Point dragToolCompensation(StringBuilder gcodeString, float finalx, float finaly)
		{
		...
		private void Move(StringBuilder gcodeString, int gnr, float x, float y, bool applyFeed, string cmt)
		{
			Move(gcodeString, gnr, x, y, null, applyFeed, cmt);
		}
		private void Move(StringBuilder gcodeString, int gnr, float x, float y, float? z, bool applyFeed, string cmt)
		{
			if (gnr == 0)
			{
				segLastFinalX = segFinalX; segLastFinalY = segFinalY;
		...
		public void splitLine(StringBuilder gcodeString, int gnr, float x1, float y1, float x2, float y2, float maxStep, bool applyFeed, string cmt = "")
		{
			float dx = x2 - x1;
			float dy = y2 - y1;
		...
		public void Arc(StringBuilder gcodeString, int gnr, Point coordxy, Point coordij, string cmt = "", bool avoidG23 = false)
		{ MoveArc(gcodeString, gnr, (float)coordxy.X, (float)coordxy.Y, (float)coordij.X, (float)coordij.Y, applyXYFeedRate, cmt, avoidG23); }
		public void Arc(StringBuilder gcodeString, int gnr, float x, float y, float i, float j, string cmt = "", bool avoidG23 = false)
		{ MoveArc(gcodeString, gnr, x, y, i, j, applyXYFeedRate, cmt, avoidG23); }
		private void MoveArc(StringBuilder gcodeString, int gnr, float x, float y, float i, float j, bool applyFeed, string cmt = "", bool avoidG23 = false)
		{
			string feed = "";
		...
		public void splitArc(StringBuilder gcodeString, int gnr, float x1, float y1, float x2, float y2, float i, float j, bool applyFeed, string cmt = "")
		{
		...
		public StringBuilder gcodeString = new StringBuilder();
		...
		//public GCodeFromSVG(GrblCore core)
  //      {
		//	grblCore = core;
		//}

		//public string ConvertFromXElement(int layerIdx, GrblCore core)
		//{
		//	string xml = System.IO.File.ReadAllText(core.ProjectCore.layers[layerIdx].FileName);
		//	return convertFromText(xml, core, true);
		//}
		public string ConvertFromFile(int layerIndex, GrblCore core)
		{
			//FileObject fileObject =  .GetLayerFileObject(layerIndex);
			//string xml = System.IO.File.ReadAllText(core.ProjectCore.layers[layerIdx].FileName);
			return convertFromText(Encoding.Default.GetString(core.ProjectCore.layers[layerIndex].FileObject.ByteArray),  true);
		}
		public string convertFromText(string text, bool importMM = false)
		{
			...
			XElement svgXML = XElement.Parse(text, LoadOptions.None);
			return ConvertFromSVG(svgXML);
		}

		public string ConvertFromSVG(XElement svgCode)
		{
			Reset();          // initialize GCode creation (get stored settings for export)
			setRapidNum(Setting.App.DisableG0fastskip ? 1 : 0);
			PutInitialCommand(gcodeString);

			startConvert(svgCode);

			PutFinalCommand(gcodeString);
		...
		/// </summary>
		public void startConvert(XElement svgCode)
		{
			countSubPath = 0;
		...
		/// </summary>
		private void parseGroup(XElement svgCode, int level, bool reverseOrder = true)
		{
            foreach (XElement groupElement in reverseOrder ? svgCode.Elements(nspace + "g").Reverse() : svgCode.Elements(nspace + "g"))
			{
				if (svgComments)
				{
			...
			foreach (var form in forms)
			{
				foreach (var pathElement in svgCode.Elements(nspace + form))
				{
					if (pathElement != null)
					{
						...
						{
							gcodePenUp("1st shape");
							startFirstElement = false;
						}
		...
		/// </summary>
		private void parsePath(XElement svgCode, int level, bool reverseOrder = true)
		{
			foreach (var pathElement in reverseOrder ? svgCode.Elements(nspace + "path").Reverse() : svgCode.Elements(nspace + "path"))
			{
				if (pathElement != null)
				{
					offsetX = 0;// (float)matrixElement.OffsetX;
		...
		// helper functions
		private float fsqrt(float x) 
		{ 
			return (float)Math.Sqrt(x); 
		}
		private float fvmag(float x, float y) 
		{ 
		...
		private void gcodeStartPath(float x, float y, string cmt)
		{
			...
			gcodePenUp(cmt);
			MoveToRapid(gcodeString, coord, cmt);
		...
		private void gcodeStopPath(string cmt)
		{
			if (gcodeReduce)
			{
				if ((lastSetGCX != lastGCX) || (lastSetGCY != lastGCY)) // restore last skipped point for accurat G2/G3 use
					MoveTo(gcodeString, new System.Windows.Point(lastGCX, lastGCY), "restore Point");
			}
			gcodePenUp(cmt);
		}
		...
		private void gcodeArcToCCW(float x, float y, float i, float j, string cmt, bool avoidG23 = false)
		{
			...
			if (gcodeReduce && isReduceOk)      // restore last skipped point for accurat G2/G3 use
			{
				if ((lastSetGCX != lastGCX) || (lastSetGCY != lastGCY))
					MoveTo(gcodeString, new System.Windows.Point(lastGCX, lastGCY), cmt);
			}
			Arc(gcodeString, 3, coordxy, coordij, cmt, avoidG23);
		}
