...
namespace GRBLLibrary
{
	...
	public partial class GrblFile : IEnumerable<GrblCommand>
	{ 
		...
		public delegate void OnFileLoadedDlg(long elapsed);
		...
		public List<GrblCommand> grblCommands = new List<GrblCommand>();




		//private ProgramRange mRange = new ProgramRange();
		//private ProgramRange xlayerRange;
		//public ProgramRange layerRange 
		//{
		//	get { return xlayerRange; }
		//	set { 
		//		this.xlayerRange = value; 
		//	}
		//}
		...
		public GrblFile()
        {
			//layerRange = new ProgramRange();
			//layerRange.UpdateXYRange(0,0,0,0,false);

		}



		//public GrblFile(ProgramRange range, decimal x, decimal y, decimal x1, decimal y1)
		public GrblFile(ProgramRange range)
		{
//			layerRange = range;
			//layerRange.UpdateXYRange(new GrblCommand.Element('X', x), new GrblCommand.Element('Y', y), false);
			//layerRange.UpdateXYRange(new GrblCommand.Element('X', x1), new GrblCommand.Element('Y', y1), false);
		}
		...
		public void SaveGCODE(string filename, bool header, bool footer, bool between, int cycles, GrblCore core)
		{
			try
			{
				using (System.IO.StreamWriter sw = new System.IO.StreamWriter(filename))
				{
					if (header)
                    {
						EvaluateAddLines(core, sw, Setting.App.GCodeCustomHeader); // ", GrblCore.GCODE_STD_HEADER));
                    }
					...
					for (int i = 0; i < cycles; i++)
					{
                        ...
                        {
							EvaluateAddLines(core, sw, Setting.App.GCodeCustomPasses); // ", GrblCore.GCODE_STD_PASSES));
                        }
					}

					if (footer)
                    {
						EvaluateAddLines(core, sw, Setting.App.GCodeCustomFooter);// ", GrblCore.GCODE_STD_FOOTER));
                    }
		...
		private void EvaluateAddLines(GrblCore core, System.IO.StreamWriter sw, string lines)
		{
		...
		//      public void LoadImportedSVG(GrblCore core, int layerIndex)
		//{
		//	RiseOnFileLoading();
		...
		//	#region build gCode (wow, its a mess)



		//	SvgConverter.GCodeFromSVG converter = new SvgConverter.GCodeFromSVG();

		//	converter.GCodeXYFeed = core.ProjectCore.layers[layerIndex].Config.BorderSpeed;
		//	converter.UseLegacyBezier = !core.ProjectCore.layers[layerIndex].Config.UseSmartBezier;
		...
		//          {
		//		converter.gcodeSpindleSpeed = core.ProjectCore.layers[layerIndex].Config.PowerMax;
		//	}
		//          else
		//          {
		//		converter.gcodeSpindleSpeed = (float)core.Configuration.MaxPWM;
		//	}
		//	converter.Reset();
		...
		//	converter.startConvert(core.ProjectCore.layers[layerIndex].XElement);
		...
		//	int passes = core.ProjectCore.layers[layerIndex].Config.Passes;
		...
		//	Console.WriteLine($"*** xxxx Layer[-]***: X:{layerRange.DrawingRange.X.Min}-{layerRange.DrawingRange.X.Max} Y:{layerRange.DrawingRange.Y.Min}-{layerRange.DrawingRange.Y.Max}");



		//	for (int i = 0; i < core?.ProjectCore?.layers?.Count; i++)
		//	{
		//		Console.WriteLine($"*** BEFORE Layer[{i}]***: X:{core.ProjectCore.layers[i].LayerGRBLFile.layerRange.DrawingRange.X.Min}-{core.ProjectCore.layers[i].LayerGRBLFile.layerRange.DrawingRange.X.Max} Y:{core.ProjectCore.layers[i].LayerGRBLFile.layerRange.DrawingRange.Y.Min}-{core.ProjectCore.layers[i].LayerGRBLFile.layerRange.DrawingRange.Y.Max}");
		//	}
		...
		//	for (int i = 0; i < core?.ProjectCore?.layers?.Count; i++)
		//	{
		//		Console.WriteLine($"*** ATER Layer[{i}]***: X:{core.ProjectCore.layers[i].LayerGRBLFile.layerRange.DrawingRange.X.Min}-{core.ProjectCore.layers[i].LayerGRBLFile.layerRange.DrawingRange.X.Max} Y:{core.ProjectCore.layers[i].LayerGRBLFile.layerRange.DrawingRange.Y.Min}-{core.ProjectCore.layers[i].LayerGRBLFile.layerRange.DrawingRange.Y.Max}");
		//	}
		...
		public class DSegment : ColorSegment
		{
		...
		public class VSeparator : ColorSegment
		{
		...
		public bool RasterFilling(ImageProcessor.Direction dir)
		{
		...
		public bool VectorFilling(ImageProcessor.Direction dir)
		{
		...
		public bool TimeConsumingFilling(ImageProcessor.Direction dir)
		{
			return
			dir == ImageProcessor.Direction.NewCross ||
			dir == ImageProcessor.Direction.NewDiagonalCross ||
		...
		public void LoadImagePotrace(Bitmap bmp, int layerIdx, bool UseSpotRemoval, int SpotRemoval, bool UseSmoothing, decimal Smoothing, bool UseOptimize, decimal Optimize, bool useOptimizeFast, L2LConf c, GrblCore core)
		{
			skipcmd = Setting.App.DisableG0fastskip ? "G1" : "G0";
			...
			if (RasterFilling(c.dir))
			{
				using (Bitmap ptb = new Bitmap(bmp.Width, bmp.Height))
				{
					using (Graphics g = Graphics.FromImage(ptb))
					{
						...
						{
							if (c.SupportPWM)
								grblCommands.Add(new GrblCommand(String.Format("{0} S0", c.LaserOn))); //laser on and power to zero
							else
								grblCommands.Add(new GrblCommand(String.Format($"{c.LaserOff} S{core.configuration.MaxPWM}"))); //laser off and power to max power

							//set speed to markspeed
							// For marlin, need to specify G1 each time :
							// list.Add(new GrblCommand(String.Format("G1 F{0}", c.markSpeed)));
							grblCommands.Add(new GrblCommand(String.Format("F{0}", c.markSpeed)));

							c.vectorfilling = true;
							ImageLine2Line(resampled, c);

							//laser off
							grblCommands.Add(new GrblCommand(c.LaserOff));
						}
					}
				}
			}
			...
			if (supportPWM)
				grblCommands.Add(new GrblCommand($"{c.LaserOn} S0"));   //laser on and power to 0
			else
				grblCommands.Add(new GrblCommand($"{c.LaserOff} S{core.configuration.MaxPWM}"));   //laser off and power to maxPower
			...
			if (flist != null)
			{
				...
				if (supportPWM)
					...
					gc.AddRange(Potrace.Export2GCode(flist, c.oX, c.oY, c.res, c.LaserOn, c.LaserOff, bmp.Size, skipcmd));

				grblCommands.Add(new GrblCommand(String.Format("F{0}", c.markSpeed)));
				foreach (string code in gc)
					grblCommands.Add(new GrblCommand(code));
			}
			...
			if (plist != null) //always true
			{
				...
				if (supportPWM)
					...
					gc.AddRange(Potrace.Export2GCode(plist, c.oX, c.oY, c.res, c.LaserOn, c.LaserOff, bmp.Size, skipcmd));

				// For marlin, need to specify G1 each time :
				//list.Add(new GrblCommand(String.Format("G1 F{0}", c.borderSpeed)));
				grblCommands.Add(new GrblCommand(String.Format("F{0}", c.borderSpeed)));
				foreach (string code in gc)
					grblCommands.Add(new GrblCommand(code));
			}
			...
			//laser off (superflua??)
			if (supportPWM)
				grblCommands.Add(new GrblCommand(c.LaserOff));  //necessaria perché finisce con solo S0
		...
		private void RiseOnFileLoaded(long elapsed)
		{
		...
		public void LoadImageL2L(Bitmap bmp, int layerIdx, L2LConf c, GrblCore core)
		{
			...
			grblCommands.Add(new GrblCommand(String.Format("{0} X{1} Y{2} F{3}", skipcmd, formatnumber(c.oX), formatnumber(c.oY), c.markSpeed)));
			if (c.SupportPWM)
            {
				grblCommands.Add(new GrblCommand(String.Format("{0} S0", c.LaserOn))); //laser on and power to zero
            }
		...
		private void ImageLine2Line(Bitmap bmp, L2LConf c)
		{
			...
			foreach (ColorSegment seg in segments)
			{
				...
				if (seg.IsSeparator && !fast) //fast = previous segment contains S0 color
				{
                    ...
                    {
						temp.Add(new GrblCommand(c.LaserOff)); //laser off
                    }
				}
				...
				try
				{
					...
					if (oldcumulate && !cumulate) //cumulate down front -> flush
					{
						if (c.SupportPWM)
							...
							rv.Add(new GrblCommand(string.Format("{0} X{1} Y{2} {3}", skipcmd, formatnumber((double)curX), formatnumber((double)curY), c.LaserOff)));
		...
		private double SquareDistance(dPoint a, dPoint b)
		{
			double dX = b.X - a.X;
			double dY = b.Y - a.Y;
			return ((dX * dX) + (dY * dY));
		}
		private double SquareDistanceZero(dPoint a)
		{
		...
		public int GCodeLineCount
		{ get { return grblCommands.Count; } }

		public List<string> GetCommands()
        {
			return grblCommands.Select(n => n.Command).ToList();
        }
		...
		//	get
		//	{
		//		if (!layerRange.DrawingRange.ValidRange)
		//			return CartesianQuadrant.Unknown;
		//		else if (layerRange.DrawingRange.X.Min >= 0 && layerRange.DrawingRange.Y.Min >= 0)
		//			return CartesianQuadrant.I;
		//		else if (layerRange.DrawingRange.X.Max <= 0 && layerRange.DrawingRange.Y.Min >= 0)
		//			return CartesianQuadrant.II;
		//		else if (layerRange.DrawingRange.X.Max <= 0 && layerRange.DrawingRange.Y.Max <= 0)
		//			return CartesianQuadrant.III;
		//		else if (layerRange.DrawingRange.X.Min >= 0 && layerRange.DrawingRange.Y.Max <= 0)
		...
		//internal void DrawOnGraphics(Graphics g, Size size, float zoom, Color color)
		//{
		//	if (!mGlobalRange.MovingRange.ValidRange)
  //          {
		//		return;
  //          }

		//	GrblCommand.StatePositionBuilder spb = new GrblCommand.StatePositionBuilder();
		//	ProgramRange.XYRange scaleRange = mGlobalRange.MovingRange;
		...
		//	// TOO: Move out, dont re-draw for each image
		//	//ScaleAndPosition(g, size, scaleRange, zoom);

		//	//DrawJobPreview(g, spb, zoom, color, true);
		//	DrawJobRange(g, size, zoom);

		//	//ScaleAndPosition(g, size, scaleRange, zoom);
		//}

		//internal void ReScale(Graphics g, Size size, float zoom)
		//{
		//	if (!mGlobalRange.MovingRange.ValidRange)
		//	{
		//		return;
		...
		//public void DrawJobPreview(Graphics g, GrblCommand.StatePositionBuilder spb, float zoom, Color lineColor, bool firstline)
		//public void DrawJobPreview(Graphics g, GrblCommand.StatePositionBuilder spb, float zoom, Color startColor, Color defaultColor)
		//{
		...
		//			if (spb.TrueMovement())
		//			{
		//				Color linecolor = Color.FromArgb(spb.GetCurrentAlpha(mGlobalRange.SpindleRange), firstline ? startColor : spb.LaserBurning ? defaultColor : ColorScheme.PreviewOtherMovement);
  ...
  //                          }
		...
		internal void LoadImageCenterline(Bitmap bmp, int layerIdx, bool useCornerThreshold, int cornerThreshold, bool useLineThreshold, int lineThreshold, L2LConf conf, GrblCore core)
		{
			...
			LaserGRBLPlus.SvgConverter.GCodeFromSVG converter = new LaserGRBLPlus.SvgConverter.GCodeFromSVG()
            {
		...
		/// <summary>
		/// analyze the file and build global range and timing for each command
		/// </summary>
		public void Analyze() 
		{
			// use gcode
			...
			//layerRange.ResetRange();
			//layerRange.UpdateXYRange("X0", "Y0", false);
			...
			//		layerRange.UpdateSRange(spb.S);

			//		if (spb.LastArcHelperResult != null)
   //                 {
			//			layerRange.UpdateXYRange(spb.LastArcHelperResult.BBox.X, spb.LastArcHelperResult.BBox.Y, spb.LastArcHelperResult.BBox.Width, spb.LastArcHelperResult.BBox.Height, spb.LaserBurning);
   //                 }
			//		else
   //                 {
			//			layerRange.UpdateXYRange(spb.X, spb.Y, spb.LaserBurning);
   //                 }
		...
		////public static ScaleAndPosition(Graphics g, Size s, ProgramRange.XYRange scaleRange, float zoom)
		//{
		//	g.ResetTransform();
		//	float margin = 10;
		...
		//	//draw job range
		//	if (mGlobalRange.DrawingRange.ValidRange)
		//	{
		...
		//			g.DrawLine(pen, -wSize.Width, (float)mGlobalRange.DrawingRange.Y.Min, wSize.Width, (float)mGlobalRange.DrawingRange.Y.Min);
		//			g.DrawLine(pen, -wSize.Width, (float)mGlobalRange.DrawingRange.Y.Max, wSize.Width, (float)mGlobalRange.DrawingRange.Y.Max);
		//			g.DrawLine(pen, (float)mGlobalRange.DrawingRange.X.Min, -wSize.Height, (float)mGlobalRange.DrawingRange.X.Min, wSize.Height);
		//			g.DrawLine(pen, (float)mGlobalRange.DrawingRange.X.Max, -wSize.Height, (float)mGlobalRange.DrawingRange.X.Max, wSize.Height);
		...
		//			string format = "0";
		//			if (mGlobalRange.DrawingRange.Width < 50 && mGlobalRange.DrawingRange.Height < 50)
		//				format = "0.0";

		//			DrawString(g, zoom, 0, mGlobalRange.DrawingRange.Y.Min, mGlobalRange.DrawingRange.Y.Min.ToString(format), false, true, !right, false, ColorScheme.PreviewText);
		//			DrawString(g, zoom, 0, mGlobalRange.DrawingRange.Y.Max, mGlobalRange.DrawingRange.Y.Max.ToString(format), false, true, !right, false, ColorScheme.PreviewText);
		//			DrawString(g, zoom, mGlobalRange.DrawingRange.X.Min, 0, mGlobalRange.DrawingRange.X.Min.ToString(format), true, false, false, top, ColorScheme.PreviewText);
		//			DrawString(g, zoom, mGlobalRange.DrawingRange.X.Max, 0, mGlobalRange.DrawingRange.X.Max.ToString(format), true, false, false, top, ColorScheme.PreviewText);
		//		}
		//	}
		...
		//		if (mGlobalRange.DrawingRange.ValidRange && mGlobalRange.DrawingRange.Width < 50 && mGlobalRange.DrawingRange.Height < 50)
		//			format = "0.0";
		...
		{ return grblCommands.GetEnumerator(); }


		public System.Collections.IEnumerator GetEnumerator()
		{ return grblCommands.GetEnumerator(); }

		//public ProgramRange Range 
		//{ 
		//	get
		//	{ 
		//		return layerRange; 
