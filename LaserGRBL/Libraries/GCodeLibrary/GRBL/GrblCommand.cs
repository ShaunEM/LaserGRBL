...
namespace GRBLLibrary
{
	...
	{
		private string grblCommandString;		
		...
		public GrblCommand(string grblCommandString, int repeat = 0, bool preservecase = false)
		{ 
			this.grblCommandString = grblCommandString.Trim();
			if (!preservecase)
			{
				this.grblCommandString = this.grblCommandString.ToUpper();
			}
			repeatCount = repeat; 
		}
		...
		public GrblCommand(IEnumerable<GrblElement> elements)
		{
            ...
            {
				grblCommandString = grblCommandString + e.ToString() + " ";
            }
			grblCommandString = grblCommandString.ToUpper().Trim();
		}
		...
		public void BuildHelper()
		{
			...
			try
			{
				...
				{
					foreach (char c in grblCommandString)
					{
						...
						if (!comment)
						{
							if (Char.IsLetter(c))
							{
								if (cmd != '\0') //chiudi il comando precedente
									Add(new GrblElement(cmd, float.Parse(num, System.Globalization.NumberFormatInfo.InvariantInfo)));
					...
					grblCommandString = sb.ToString();

					if (cmd != '\0')
						Add(new GrblElement(cmd, float.Parse(num, System.Globalization.NumberFormatInfo.InvariantInfo))); //aggiungi l'ultimo
				}
			}
			catch { }
		}
		...
		{ 
			...
			{
				if (CanCompress)
					return grblCommandString.Trim(trimarray).Replace(" ","") + '\n';  //strip spaces
				else
					return grblCommandString.Trim(trimarray) + '\n';  //send it "as is"
			} 
		}
		...
		{ get { return grblCommandString.StartsWith("$"); } }
		
		public bool IsEmpty
		{get{return grblCommandString.Length == 0;}}

		public bool IsWriteEEPROM
		{ get { return IsGrblCommand && IsSetConf; } } //maybe need to add G10/G28.1/G30.1 ?

		private bool IsSetConf
		{ get { return GrblSettings.IsSetConf(grblCommandString); } }
		...
		public GrblElement M { get { return GetElement('M'); } }
		...
		{  return repeatCount == 0 ? Command : String.Format("{0} (Retry {1})", Command, repeatCount); } 
