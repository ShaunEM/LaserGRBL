    ...
    public partial class GrblCommand
	{
		public class StatePositionBuilder : StateBuilder
		{
			...
			public class CumulativeElement : GrblElement
			{
				...
				public void Update(GrblElement e, bool Absolute, float offset)
				{
					mPrevious = mNumber;
			...
			private float mWcoX = 0;
			private float mWcoY = 0;
			private float mWcoZ = 0;
			...
			public TimeSpan AnalyzeCommand(GrblCommand cmd, double maxRateX = 0)
			{
				...
				TimeSpan rv = maxRateX > 0 ? ComputeExecutionTime(cmd, maxRateX) : TimeSpan.Zero;
            ...
            //private TimeSpan ComputeExecutionTime(GrblCommand cmd, GrblSettings conf)
            private TimeSpan ComputeExecutionTime(GrblCommand cmd, double maxRateX)
            {
				float f = cmd is JogCommand && cmd.F != null ? cmd.F.Number : mCurF.Number;

				if (G0 && cmd.IsLinearMovement)
				{
					return TimeSpan.FromMinutes((double)GetSegmentLenght(cmd) / maxRateX); //todo: use a better computation of xy if different x/y max speed
				}
				else if (G1G2G3 && cmd.IsMovement && f != 0)
				{
					return TimeSpan.FromMinutes((double)GetSegmentLenght(cmd) / (double)Math.Min(f, maxRateX));
				}
				else if (cmd.IsPause)
				{
			...
			private float GetSegmentLenght(GrblCommand cmd)
			{
				...
				if (cmd.IsLinearMovement)
					...
					return (float)GetArcHelper(cmd).AbsLenght;
				else
					return 0;
			}
			...
			{ 
				get 
				{ 
					return (!supportPWM || S.Number > 0) && (SpindleState.Number == 3 || (SpindleState.Number == 4 && MotionMode.Number != 0)); 
				} 
			}
