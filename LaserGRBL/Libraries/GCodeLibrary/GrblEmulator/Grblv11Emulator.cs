...
namespace LaserGRBLPlus.GrblEmulator
{
	...
	public class Grblv11Emulator
	{
		...
		public Grblv11Emulator(SendMessage sendFunc)
		{
			...
			RX = new Tools.ThreadObject(ManageRX, 10, true, "Emulator RX", null);
			TX = new Tools.ThreadObject(ManageTX, 10, true, "Emulator TX", null);
			mSendFunc = sendFunc;

			conf = (GrblSettings)Tools.Serializer.ObjFromFile(filename);
			if (conf == null)
			{
				conf = new GrblSettings(
				   new GrblVersionInfo(1, 1, '#'),
		...
		{ 
			return GrblSettings.IsSetConf(p); 
		}
		...
		private void EmulateCommand(GrblCommand cmd)
		{
			if (!mCheck)
			{
				try
				{
					TimeSpan cmdTime = SPB.AnalyzeCommand(cmd, conf.MaxRateX);
					toSleep += cmdTime;
					//toSleep = TimeSpan.FromMilliseconds(10);
